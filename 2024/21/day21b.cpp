...
vector<string> paths(char c, char d,
	...
	for (auto [dir, dirc] : directions) {
		...
		if (distance(cx, cy, dx, dy) < distance(i.first, i.second, dx, dy))
			continue;
		const auto next = paths(rev[i], d, m, rev);
		for (auto n : next)
			v.push_back(dirc+n);
	}
...
map<tuple<char, char, int>, int64_t> t;
int64_t pushes(char c, char d,
    map<char, pair<int, int>>& m, map<pair<int, int>, char>& r, int n) {
	if (t.count(make_tuple(c, d, n)))
		return t[make_tuple(c, d, n)];
	const auto p = paths(c, d, m, r);
	...
	for (const auto& path : p) {
		if (n == 0) {
			...
			for (unsigned i = 0; i < q.size()-1; i++)
				u += pushes(q[i], q[i+1], m_dir, rev_dir, n-1);
			ret = min(ret, u);
		}
	}
	return t[make_tuple(c, d, n)] = ret;
}
...
int main() {
	...
	while (getline(cin, line)) {
		...
		for (unsigned i = 0; i < line.size()-1; i++)
			comp += pushes(line[i], line[i+1], m_num, rev_num, 25);
		tot += comp * num;
	}
	cout << tot << endl;
	return 0;
